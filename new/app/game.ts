...
async function boot(): Promise<void> {
  try {
    const me = await apiJson<any>("/api/me.php", { method: "GET" });
    dlog("me.php OK:", me);
  } catch (e) {
  ...
  try {
    mm = await apiJson<MMResp>("/api/game/matchmake.php", { method: "POST" });
  } catch (e) {
  ...
  ws.onclose = (ev) => {
    ...
    dlog("ws close", { code: ev.code, reason: ev.reason, wasClean: ev.wasClean });
  };
  ...
  ws.onmessage = (event: MessageEvent) => {
    ...
    try {
      ...
      dlog("ws msg (non-json):", raw.slice(0, 200));
      return;
    }

    // High-signal log of message type + a few fields
    if (DEBUG) {
      const t = msg?.type;
      if (t === "state") {
        // do not spam full state every time; log occasionally
    ...
    if (msg.type === "state") {
      ...
      // Sanity checks that catch “game not playing” instantly
      if (!next || !next.ball || !next.paddles || !next.score) {
        dlog("BAD state shape:", next);
        return;
      }

      // Check if ball/paddles are actually changing
      if (lastBall) {
        const dx = Math.abs(next.ball.x - lastBall.x);
        const dy = Math.abs(next.ball.y - lastBall.y);
        // If active but ball never moves -> server tick not running or state not updating
      ...
      // Extra: detect role mismatch with player list (super common)
      if (DEBUG && myRole) {
...
function render(): void {
  ...
  // draw overlay last so it’s on top
  drawDebugOverlay();
