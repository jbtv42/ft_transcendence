...
export function renderTournamentView(root: HTMLElement): void {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const players: PlayerInput[] = inputs.map((input, index) => ({
      id: index + 1,
...
function startBracket(root: HTMLElement, players: PlayerInput[]): void {
  if (players.length !== 4) {
    throw new Error("Tournament expects exactly 4 players.");
  }

  let roundIndex = 0;
  const semiWinners: PlayerInput[] = [];
  ...
  function startMatch(left: PlayerInput, right: PlayerInput, label: string) {
    renderGameView(root, {
      ...
      onGameEnd: (state: GameState) => {
        ...
        if (roundIndex === 1) {
          setTimeout(() => {
          startMatch(players[2], players[3], "Semi-final 2"); }, 5000);
        } else if (roundIndex === 2) {
          setTimeout(() => {
          startMatch(semiWinners[0], semiWinners[1], "Final"); }, 5000);
        } else if (roundIndex === 3) {
          setTimeout(() => {
          renderTournamentResult(root, winner, finalLoser, players); }, 5000);
        }
      },
    });
  ...
  setTimeout (() => {
  startMatch(players[0], players[1], "Semi-final 1"); }, 5000);
}

function renderTournamentResult(
  root: HTMLElement,
  champion: PlayerInput,
  finalLoser: PlayerInput | null,
